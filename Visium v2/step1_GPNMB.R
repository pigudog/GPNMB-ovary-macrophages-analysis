...
rm(list = ls())
gc()
load("../scRNA_estrous/rdata/ov_scRNA_myelo.rda")
...
save(scRNA_myelo,file = "./rdata/scRNA_myelo_gpnmb.rda")
...
for (i in 1:dim(res)[1]) {
  ...
  gene_names = paste(unique(data[data$ENTREZID %in% arr,]$SYMBOL), collapse="/")
  res[i,"geneID"] = gene_names
}

head(res)

write.csv(res,file = "files/gpnmb_mac_gobp.csv")
...
p <- ggplot(data = dt, aes(x = -log10(pvalue), y = rev(Description), fill = Cluster)) +
  scale_fill_manual(values =c("#AFC2D9", "#f1707d")) +
  geom_bar(stat = "identity", width = 0.5, alpha = 0.8) +
  scale_x_continuous(expand = c(0,0)) + # 调整柱子底部与y轴紧贴
  labs(x = "-Log10(pvalue)", y = "Gpnmb+Mac       Gpnmb-Mac", title = "GOBP Pathway enrichment") +
  # x = 0.61 用数值向量控制文本标签起始位置
  geom_text(size=3.8, aes(x = 0.05, label = Description), hjust = 0) + # hjust = 0,左对齐
  geom_text(size=3, aes(x = 0.05, label = geneID), hjust = 0, vjust = 2.5, color=rep(c("#AFC2D9", "#f1707d"),each=5)) + # hjust = 0,左对齐
  theme_classic() + 
  mytheme +
  NoLegend()
...
ggsave(filename = "scRNA/GOBP.pdf", width = 5.2, height = 6, plot = p)
